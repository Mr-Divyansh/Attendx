...
type Classroom = { id: string; name: string; course?: string | null; section?: string | null; teachingMode?: string | null; room?: string | null; joinCode: string; inviteToken: string; subject?: Subject | null; semester?: { id: string; name: string } | null; year?: number | null; durationYears?: number | null; expiresAt?: string | null; expired?: boolean; status?: string; members: { id: string; status: string; joinedAt: string; student: { id?: string; fullName: string; rollNo: string; email?: string | null; user?: { email: string } } }[]; schedules?: Schedule[] }
...
function ClassDetailDialog({ classroom, onClose, onChanged }: { classroom: Classroom | null; onClose: () => void; onChanged: () => void }) {
  const [day, setDay] = useState('Mon'); const [startTime, setStart] = useState('10:00'); const [endTime, setEnd] = useState('11:00'); const [busy, setBusy] = useState(false)
  if (!classroom) return null
  const schedule = async (e: FormEvent) => { e.preventDefault(); setBusy(true); try { await apiFetch(`/api/classrooms/${classroom.id}/schedule`, { method: 'POST', body: JSON.stringify({ day, startTime, endTime }) }); toast.success('Schedule added'); onChanged() } catch (x) { toast.error(x instanceof Error ? x.message : 'Unable to add schedule') } finally { setBusy(false) } }
  const respond = async (memberId: string, action: 'approve' | 'reject') => { setBusy(true); try { await apiFetch('/api/classrooms/members', { method: 'POST', body: JSON.stringify({ memberId, action }) }); toast.success(action === 'approve' ? 'Request accepted. The student can now access the classroom.' : 'Request rejected'); onChanged() } catch (x) { toast.error(x instanceof Error ? x.message : 'Unable to update request') } finally { setBusy(false) } }
  const removeStudent = async (memberId: string, studentName: string) => { if (!confirm(`Remove ${studentName} from this classroom? Their account will not be deleted.`)) return; setBusy(true); try { await apiFetch('/api/classrooms/members', { method: 'POST', body: JSON.stringify({ memberId, action: 'reject' }) }); toast.success('Student removed from this classroom'); onChanged() } catch (x) { toast.error(x instanceof Error ? x.message : 'Unable to remove student') } finally { setBusy(false) } }
  const active = classroom.members.filter(m => m.status === 'ACTIVE')
  const pending = classroom.members.filter(m => m.status === 'PENDING')
  return <Dialog open={!!classroom} onOpenChange={v => !v && onClose()}><DialogContent className="max-w-3xl"><DialogHeader><DialogTitle className="flex flex-wrap items-center gap-2">{classroom.name}{classroom.expired ? <Badge className="border-transparent bg-red-500/15 text-red-600 dark:text-red-400">Expired</Badge> : null}</DialogTitle></DialogHeader><div className="grid gap-6 md:grid-cols-2"><div><p className="text-sm text-muted-foreground">{classroom.course || 'Course'} · {classroom.subject?.name || 'Subject'} · {classroom.section || 'Section'}{classroom.year ? ` · Year ${classroom.year}` : ''}</p><p className="mt-2 text-sm text-muted-foreground">{classroom.semester?.name ? `${classroom.semester.name} · ` : ''}Duration: {classroom.durationYears ? `${classroom.durationYears} Year${classroom.durationYears > 1 ? 's' : ''}` : '—'}{classroom.expiresAt ? ` · Ends ${new Date(classroom.expiresAt).toLocaleDateString()}` : ''}</p>{pending.length > 0 && <><h3 className="mt-5 font-semibold flex items-center gap-2"><Inbox className="size-4 text-primary" />Student Requests ({pending.length})</h3><div className="mt-2 max-h-52 overflow-auto rounded-lg border">{pending.map(m => <div className="flex items-center justify-between gap-2 border-b px-3 py-2 text-sm last:border-0" key={m.id}><div className="min-w-0"><span className="font-medium">{m.student.fullName}</span><p className="text-xs text-muted-foreground">Roll No: {m.student.rollNo}{m.student.email ? ` · ${m.student.email}` : ''}</p><p className="text-xs text-muted-foreground">Requested {new Date(m.joinedAt).toLocaleString()}</p></div><div className="flex gap-2 shrink-0"><Button size="sm" variant="outline" disabled={busy} onClick={() => respond(m.id, 'reject')} aria-label={`Reject ${m.student.fullName}`} title="Reject request"><X className="size-4" /></Button><Button size="sm" disabled={busy} onClick={() => respond(m.id, 'approve')} aria-label={`Accept ${m.student.fullName}`} title="Accept request"><Check className="size-4" /></Button></div></div>)}</div></>}<h3 className="mt-5 font-semibold">Students ({active.length} active)</h3><div className="mt-2 max-h-52 overflow-auto rounded-lg border">{active.length ? active.map(m => <div className="flex items-center justify-between gap-2 border-b px-3 py-2 text-sm last:border-0" key={m.id}><div><span>{m.student.rollNo} · {m.student.fullName}</span><Badge className="ml-2" variant="secondary">Active</Badge></div><Button aria-label={`Remove ${m.student.fullName}`} title="Remove from class" variant="ghost" size="icon" disabled={busy} onClick={() => removeStudent(m.id, m.student.fullName)}><UserMinus className="size-4 text-destructive" /></Button></div>) : <p className="p-3 text-sm text-muted-foreground">No active students yet.</p>}</div><p className="mt-4 text-sm">Invite code: <span className="font-mono font-semibold">{classroom.joinCode}</span></p></div><div><h3 className="font-semibold">Timetable</h3><div className="mt-2 space-y-2">{classroom.schedules?.length ? classroom.schedules.map(s => <div className="rounded-md border px-3 py-2 text-sm" key={s.id}>{s.day} · {displayTime(s.startTime)} – {displayTime(s.endTime)}</div>) : <p className="text-sm text-muted-foreground">No schedule yet.</p>}</div><form className="mt-4 grid gap-2" onSubmit={schedule}><Select value={day} onValueChange={setDay}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{DAYS.map(d => <SelectItem key={d} value={d}>{d}</SelectItem>)}</SelectContent></Select><div className="grid grid-cols-2 gap-2"><Input type="time" value={startTime} onChange={e => setStart(e.target.value)} /><Input type="time" value={endTime} onChange={e => setEnd(e.target.value)} /></div><Button disabled={busy}><Plus className="mr-2 size-4" />Add schedule</Button></form></div></div></DialogContent></Dialog>
}
